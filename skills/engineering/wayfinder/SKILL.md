---
name: wayfinder
description: Plan a huge chunk of work — more than one agent session can hold — as a shared map of investigation tickets on your configured Work Tracker, and resolve them one at a time until the way to the destination is clear.
disable-model-invocation: true
---

A loose idea has arrived — too big for one agent session, and wrapped in fog: the way from here to the **destination** isn't visible yet. Wayfinding is about finding that way, not charging at the destination. This skill charts the way as a **shared map** on the configured Work Tracker, then works its tickets one at a time until the route is clear.
...
Each ticket is a **child item** of the map; the Work Tracker's item id is its identity. Its body is the question, sized to one 100K token agent session:
